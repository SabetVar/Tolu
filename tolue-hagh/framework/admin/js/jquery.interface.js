jQuery(document).ready(function($) {	
	...
	$('.themex_panel input,.themex_panel textarea').each(function() {
	 ...
	 /*$('.themex_panel select,.themex_panel input').live('change', function() {
  	...
  	$('.themex_panel').on('change', 'select, input', function() {
		saveButton.removeClass('disabled');
	});
    ...
    $(document).on('click', '.themex_panel .save_options:not(.disabled), .themex_panel .reset_options', function() {	
        // get options values
        var values = $('#themex_options').serialize();
        var button = $(this);
        var type = 'save';
    ...
    jQuery(document).on('click', '.themex_panel .upload_button, .repeatable-upload, .themex_meta_table .upload_button, .themex_avatar .upload_button', function(e) {
        fileInput = jQuery(this).prev('input');
        tb_show('', 'media-upload.php?post=-629834&amp;themex_uploader=1&amp;TB_iframe=true');
        header_clicked = true;
        e.preventDefault();
	...
	/*$('.add_sidebar,.remove_sidebar,.add_category,.remove_category,.add_page,.remove_page').live( 'click', function() {
  	...
  	$(document).on('click', '.add_sidebar, .remove_sidebar, .add_category, .remove_category, .add_page, .remove_page', function() {
        var values = $('#themex_options').serialize();
        var button = $(this);
        var type = '';
    ...
    $(document).on('click', '.themex_form .add_field, .themex_form .remove_field', function() {
        var values = $('#themex_options').serialize();
        var button = $(this);
        var type = '';
    ...
    $(document).on('change', '.themex_form select', function() {
        var item = $(this);
        var hiddenOption = item.parent('div').parent('div').find('div.hidden');
        if (item.find('option:selected').index() == 5) {
            hiddenOption.slideToggle(300);

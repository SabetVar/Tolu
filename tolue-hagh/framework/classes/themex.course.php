...
class ThemexCourse
{
	...
	public static function init()
	{
		...
		//ummu add Report page

		add_action('admin_menu', array(__CLASS__, 'addReport'));
	...
	public static function refresh()
	{
		...
		self::$data['icon'] = ThemexCore::getOption('course_icon', 'false');
		...
		if (self::$data['layout'] == 'three') {
			self::$data['columns'] = 4;
		} elseif (self::$data['layout'] == 'two') {
			self::$data['columns'] = 6;
		} else {
			self::$data['columns'] = 3;
		}
	...
	public static function saveCourse($ID)
	{
		...
		if (isset($_POST['add_userb']) && isset($_POST['add_userb_id'])) {

			if ($post->post_type == 'course') {

				self::addUserb($ID, intval($_POST['add_userb_id']), $_POST['user_pay']);
			} else if ($post->post_type == 'plan') {

				self::subscribeUser($ID, intval($_POST['add_userb_id']));
			}
		} else if (isset($_POST['remove_userb']) && isset($_POST['remove_userb_id'])) {

			if ($post->post_type == 'course') {

				self::removeUserb($ID, intval($_POST['remove_userb_id']));
			} else if ($post->post_type == 'plan') {

				self::unsubscribeUser($ID, intval($_POST['remove_userb_id']));
			}
		}
		...
		if (isset($_POST['add_userv']) && isset($_POST['add_userv_id'])) {

			if ($post->post_type == 'course') {

				self::addUserv($ID, intval($_POST['add_userv_id']));
			} else if ($post->post_type == 'plan') {

				self::subscribeUser($ID, intval($_POST['add_userv_id']));
			}
		} else if (isset($_POST['remove_userv']) && isset($_POST['remove_userv_id'])) {

			if ($post->post_type == 'course') {

				self::removeUserv($ID, intval($_POST['remove_userv_id']));
			} else if ($post->post_type == 'plan') {

				self::unsubscribeUser($ID, intval($_POST['remove_userv_id']));
			}
		}
		...
		if (isset($_POST['sendsmsbut']) && isset($_POST['sendtext'])) {





			$users = ThemexCore::parseMeta($ID, 'course', 'users');

			$userb = ThemexCore::parseMeta($ID, 'course', 'usersb');
			...
			foreach ($uniqUsers as $user) {
				...
				$usernumbers[] = $user->billing_phone;
			}
			...
			if (isset($_POST['add_numbers'])) {
				...
				$newNumber = explode(",", $_POST['add_numbers']);

				$usernumbers = array_merge($usernumbers, $newNumber);
			}



			SendSmS($_POST['sendtext'], $usernumbers);
		}
		...
		if (isset($_POST['sendemailbut']) && isset($_POST['sendtext'])) {
			...
			$headers[] = "From: info@toluehagh.ir";
			...
			if (isset($_POST['add_emails'])) {
				...
				$newEmail = explode(",", $_POST['add_emails']);

				$useremail = array_merge($useremail, $newEmail);
			}

			$courseName = get_the_title($ID);

			$email_heading = " اطلاعیه " . $courseName;

			$email_subject = "موسسه طلوع - اطلاعیه " . $courseName;
			...
			wc_mail('', $email_subject, $email_message, $headers);
		}
		...
		// save  Virtual user borse

		if (isset($_POST['add_uservb']) && isset($_POST['add_uservb_id'])) {

			if ($post->post_type == 'course') {

				self::addUservb($ID, intval($_POST['add_uservb_id']), $_POST['uservb_pay']);
			} else if ($post->post_type == 'plan') {

				self::subscribeUser($ID, intval($_POST['add_uservb_id']));
			}
		} else if (isset($_POST['remove_uservb']) && isset($_POST['remove_uservb_id'])) {

			if ($post->post_type == 'course') {

				self::removeUservb($ID, intval($_POST['remove_uservb_id']));
			} else if ($post->post_type == 'plan') {

				self::unsubscribeUser($ID, intval($_POST['remove_uservb_id']));
			}
		}
	}
	...
	public static function addQuestionForm($default)
	{
		...
		if (get_post_type() == 'lesson') {
			...
			$default['comment_field'] .= '<div class="field-wrapper"><textarea id="comment" name="comment" cols="45" rows="8" placeholder="' . __('متن سوال', 'academy') . '"></textarea></div></div>';
	...
	//Render Course comment

	public static function renderCourseComment($comment, $args, $depth)
	{
		...
		get_template_part('loop', 'coursecomment');
	}
	...
	public static function initCourse($ID = 0, $extended = true)
	{
		...
		self::$data['course']['categ'] = self::getCategory();





		self::$data['course']['usersv'] = ThemexCore::parseMeta($ID, 'course', 'usersv');

		self::$data['course']['usersvb'] = ThemexCore::parseMeta($ID, 'course', 'usersvb');

		self::$data['course']['usersb'] = ThemexCore::parseMeta($ID, 'course', 'usersb');



		self::$data['course']['productv'] = get_post_meta(self::$data['course']['ID'], '_course_productv', true);

		self::$data['course']['statusv'] = self::getStatusv();

		self::$data['course']['butttext'] = self::getbutttext();

		self::$data['course']['butttextaf'] = self::getbutttextaf();

		self::$data['course']['butttextcap'] = self::getbutttextcap();

		self::$data['course']['butttextvirt'] = self::getbutttextvirt();

		self::$data['course']['butttextafvirt'] = self::getbutttextafvirt();

		self::$data['course']['butttextclass'] = self::getbutttextclass();

		self::$data['course']['pricev'] = self::getPricev();

		self::$data['course']['stuview'] = self::getStudentView();
	}

	//Init current course

	public static function initCourseloop($ID = 0, $extended = true)
	{
		...
		if (isset($_POST['course_action']) && isset($_POST['course_id'])) {
			...
			switch ($_POST['course_action']) {
				...
				case 'add':
					...
					} elseif (self::$data['course']['status'] == 'free' && ThemexWoo::isActive()) {
				...
				case 'addv':
					...
					if (self::$data['course']['statusv'] == 'premium' && ThemexWoo::isActive()) {

						ThemexWoo::addProduct(self::$data['course']['productv']);
					} elseif (self::$data['course']['statusv'] == 'free' && ThemexWoo::isActive()) {

						self::addUserv(intval($_POST['course_id']));
					}
				...
				case 'removev':
					...
					self::removeUserv(intval($_POST['course_id']));
	...
	//Get Virtual course status

	public static function getStatusv()
	{



		$status = get_post_meta(self::$data['course']['ID'], '_course_statusv', true);
	...
	public static function getbutttext()
	{



		$status = get_post_meta(self::$data['course']['ID'], '_course_butttext', true);
	...
	public static function getbutttextaf()
	{



		$status = get_post_meta(self::$data['course']['ID'], '_course_butttextaf', true);
	...
	public static function getbutttextcap()
	{



		$status = get_post_meta(self::$data['course']['ID'], '_course_butttextcap', true);
	...
	public static function getbutttextclass()
	{



		$status = get_post_meta(self::$data['course']['ID'], '_course_butttextclass', true);
	...
	public static function getbutttextvirt()
	{



		$status = get_post_meta(self::$data['course']['ID'], '_course_butttextvirt', true);
	...
	public static function getbutttextafvirt()
	{



		$status = get_post_meta(self::$data['course']['ID'], '_course_butttextafvirt', true);
	...
	//Get show Student View

	public static function getStudentView()
	{


		$status = 'hidden';

		$stview = get_post_meta(self::$data['course']['ID'], '_course_stuview', true);
	...
	//Get course Product Category

	public static function getCategory()
	{

		if (self::$data['course']['status'] == 'premium') {
			$product_cat = ThemexWoo::getCategory(self::$data['course']['product']);
		} else {
			$product_cat = array(0);
		}
	...
	public static function getPrice()
	{
		...
		if (self::$data['course']['status'] == 'premium') {
			...
			$price = $product_price;
		} else if (self::$data['course']['status'] == 'private') {

			$price = __('غیر فعال', 'academy');
		}
	...
	//Get Virtual course pricev

	public static function getPricev()
	{
		...
		if (self::$data['course']['statusv'] == 'premium') {

			$product_price = ThemexWoo::getPrice(self::$data['course']['productv']);

			$price = $product_price;
		} else if (self::$data['course']['statusv'] == 'private') {
		...
		if (ThemexWoo::isActive()) {
			...
			switch ($period) {
				...
				default:
					...
					$period = round($period / 31) . ' ' . __('ماه', 'academy');
	...
	public static function getSubscriptionTime()
	{
		...
		if (!empty($subscriptions)) {
			...
			$message = __('عضویت شما در مجموع دوره ها در', 'academy') . ' ' . '"' . get_the_title($plan) . '" ';
			...
			if ($date != 0) {
				...
				if ($date == 1) {
					...
					$message .= __('روز دیگر منقضی می شود.', 'academy');
				} else {
				...
				$message .= __(' کمتر از یک روز دیگر منقضی می شود.', 'academy');
			}
		}
	...
	//Get Virtual random users

	public static function getRandomUsersv()
	{

		$users_number = intval(self::$data['users_limit']);

		$users = self::filterUsers(self::$data['course']['usersv']);
	...
	// Get topSellCourses
	public static function getTopSellCourses($number = 6)
	{

		$sellcat = get_option('themex_categorysell');

		$topcourses = get_posts(array(
...
*/
	...
	// Get Teachec Author Courses 
	public static function getAuthorCourses($teachers)
	{
		$sellcat = get_option('themex_categoryexc');
		$user_info = get_userdata($teachers);
...
*/
		...
		$author_query = new WP_Query($teachercourses);
		$author_posts = $author_query->posts;
		$author_posts = array_filter($author_posts);

		return $author_posts;
	}
	//Get Author courses

	public static function getTeacherCourses($ID, $notIn = '', $orderby = 'post_date', $numberposts = '-1')
	{
		...
		if (!empty($author_posts)) {
			...
			$course_IDs = wp_list_pluck($author_posts, 'ID');
		}
	...
	public static function getUserCourses($ID)
	{
		...
		if (!empty($user_courses) || !empty($author_courses)) {
			...
			$course_IDs = wp_list_pluck($user_courses, 'ID');
		}
	...
	//Get Virtual user courses

	public static function getUserCoursesv($ID)
	{

		$user_courses = ThemexCore::getRelatedItems($ID, 'course', 'usersv', true);
	...
	//Get Borse user courses

	public static function getUserCoursesb($ID)
	{

		$user_courses = ThemexCore::getRelatedItems($ID, 'course', 'usersb', true);
	...
	//Get Virtual Borse user courses

	public static function getUserCoursesvb($ID)
	{

		$user_courses = ThemexCore::getRelatedItems($ID, 'course', 'usersvb', true);
	...
	//Get related courses user Course (array in ID)

	public static function getRelatedCoursesUser($allcourseid, $order = 'rand')
	{
		...
		foreach ($allcourseid as $courseid) {

			$terms = wp_get_post_terms($courseid, 'course_tags');

			$teachers[] = get_post_field('post_author', $courseid);
		...
		$tagcourses = get_posts(array(

			'post_type' => 'course',

			'numberposts' => 6,

			'post__not_in' => $allcourseid,
...
*/
		...
		$teachercourses = get_posts(array(
...
*/
		...
		$courses = array_merge($tagcourses, $teachercourses);
		shuffle($courses);
	...
	public static function getRelatedCourses($ID = 0, $order = 'rand')
	{

		$categories = array();

		$excat = get_option('themex_categoryexc');

		$terms = wp_get_post_terms($ID, 'course_tags');
...
*/
	...
	public static function getRelatedLibrary($ID = 0)
	{

		$categories = array();

		$terms = wp_get_post_terms($ID, 'library_category');
	...
	public static function checkMembers()
	{
		...
		if (in_array(get_post_type(), array('lesson', 'quiz'))) {
			...
			if ((!self::isMember()) && !self::isAuthor() && !self::isMemberv() && !self::isMemberb() && !self::isMembervb() && $GLOBALS['post']->_lesson_status != 'free') {
	...
	//Check Virtual course members

	public static function hasMembersv()
	{

		if (self::$data['users'] != 'true' && !empty(self::$data['course']['usersv'])) {
	...
	//Check Borse course members

	public static function hasMembersb()
	{

		if (self::$data['users'] != 'true' && !empty(self::$data['course']['usersb'])) {
	...
	//Check Virtual Borse course members

	public static function hasMembersvb()
	{

		if (self::$data['users'] != 'true' && !empty(self::$data['course']['usersvb'])) {
	...
	//Detect course Borse member

	public static function isMemberb()
	{

		if (is_user_logged_in() && in_array(get_current_user_id(), self::$data['course']['usersb'])) {
	...
	//Detect Virtual course member

	public static function isMemberv()
	{

		if (is_user_logged_in() && in_array(get_current_user_id(), self::$data['course']['usersv'])) {
	...
	//Detect Virtual Borse course member

	public static function isMembervb()
	{

		if (is_user_logged_in() && in_array(get_current_user_id(), self::$data['course']['usersvb'])) {
	...
	public static function isMembervirt($ID)
	{
		$usersv = ThemexCore::parseMeta($ID, 'course', 'usersv');
		$usersvb = ThemexCore::parseMeta($ID, 'course', 'usersvb');
		$allusr = array_merge($usersv, $usersvb);

		if (is_user_logged_in() && in_array(get_current_user_id(), $allusr)) {
			return true;
		}
		return false;
	}
	public static function isMemberlocal($ID)
	{
		$users = ThemexCore::parseMeta($ID, 'course', 'users');
		$usersb = ThemexCore::parseMeta($ID, 'course', 'usersb');
		$allusr = array_merge($users, $usersb);
	...
	public static function completeLesson($ID = 0, $mark = 100, $protected = false)
	{
		...
		if (!isset($users[get_current_user_id()]) && (($quiz != 0 && $protected) || !$protected) && (self::isMember() || self::isMemberv() || self::isMemberb() || self::isMembervb())) {
	...
	//Add course user borse



	public static function addUserb($ID = 0, $user = null, $totalpay = 0)
	{



		$users = self::filterUsers(ThemexCore::parseMeta($ID, 'course', 'usersb'));
		...
		update_post_meta($ID, '_course_usersb', http_build_query($users));

		update_post_meta($ID, '_course_users_numberb', count($users));



		$userarray[$ID] = $totalpay;

		$oldvalue = get_user_meta($user, '_user_pay', true);

		if ($oldvalue != '') {

			$arrvalue =  $oldvalue . '&' . http_build_query($userarray);
		} else {
			$arrvalue = http_build_query($userarray);
		}

		update_user_meta($user, '_user_pay', $arrvalue);
	}



	//Add Virtual course user

	public static function addUserv($ID = 0, $user = null)
	{



		$users = self::filterUsers(ThemexCore::parseMeta($ID, 'course', 'usersv'));
		...
		update_post_meta($ID, '_course_usersv', http_build_query($users));

		update_post_meta($ID, '_course_users_numberv', count($users));
	}



	//Add Virtual course user borse

	public static function addUservb($ID = 0, $user = null, $totalpay = 0)
	{



		$users = self::filterUsers(ThemexCore::parseMeta($ID, 'course', 'usersvb'));
		...
		update_post_meta($ID, '_course_usersvb', http_build_query($users));

		update_post_meta($ID, '_course_users_numbervb', count($users));



		$userarray[$ID] = $totalpay;

		$oldvalue = get_user_meta($user, '_user_payv', true);
		...
		update_user_meta($user, '_user_payv', $arrvalue);
	}
	...
	//Remove course user borse

	public static function removeUserb($ID = 0, $user = null)
	{
	...
	//Remove Virtual course user

	public static function removeUserv($ID = 0, $user = null)
	{
	...
	//Remove Virtual course user borse

	public static function removeUservb($ID = 0, $user = null)
	{
	...
	public static function subscribeUser($ID = 0, $user = null)
	{
		...
		// Fetch plan period
		$plan_period = get_post_meta($ID, '_plan_period', true);
		$plan_days   = intval($plan_period);
		$expiration  = time() + 86400 * $plan_days;


		// Update users array
		$users[$user] = $expiration;



		// Save updated users
		$encoded_users = http_build_query($users);
		update_post_meta($ID, '_plan_users', $encoded_users);
	...
	public static function addReport()
	{
		...
		add_submenu_page('edit.php?post_type=course', __('آمار', 'academy'), __('آمار', 'academy'), 'edit_posts', 'report', array(__CLASS__, 'renderreport'));
	}
	...
	public static function renderreport()
	{
		...
		get_template_part('module', 'report');
	}
	...
	//Return Class Number Of Each Course

	public static function getClassNumber($courseID)
	{

		$classnumber = 8;

		$classnumbermeta = get_post_meta($courseID, '_course_class_number', true);

		if ($classnumbermeta && $classnumbermeta != 0) {
			$classnumber = $classnumbermeta;
		}
	...
	//Return User Present Class of Course

	public static function getUserPresentClass($userID, $courseID)
	{

		$userpresents = get_user_meta($userID, 'PresentCourse_' . $courseID, true);

		$userpresents = explode(', ', $userpresents);
	...
	//Print User Present List 	

	public static function UserPresentList($userID, $courseID)
	{



		$showpresent = get_post_meta($courseID, '_course_showpresent', true);

		if ($showpresent) {

			$classnumber = self::getClassNumber($courseID);

			$userpresents = self::getUserPresentClass($userID, $courseID);

			$classzero = 1;

			$out = '<ul class="present_list"> <div class="present_title"> حضور و غیاب شما در کلاس : </div>';

			while ($classzero <= $classnumber) {

				$checked = '';

				if (in_array($classzero, $userpresents)) {
					$checked = 'class = "checked" ';
				}

				$out .= '<li ' . $checked . ' > ' . $classzero . ' </li>';

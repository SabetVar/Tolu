...
class ThemexForm {
	...
	public static function processData() {
		...
		//send email
			...
			if(is_array(self::$data[$data['slug']]['fields'])) {
				foreach(self::$data[$data['slug']]['fields'] as $field_id=>$field) {
					$message.=self::$data[$data['slug']]['fields'][$field_id]['label'].': '.$data[$field_id].PHP_EOL.'<br/>';
					if (self::$data[$data['slug']]['fields'][$field_id]['type']=='email' ) {$sendemail = $data[$field_id] ; };
				}
			}

			//send message
			if(self::sendEmail($message, $data['slug'], $sendemail)) {
	...
	public static function sendEmail($message, $slug='' ,$sendemail='info@toluehagh.ir') {		
		//headers				
		$headers = "MIME-Version: 1.0" . PHP_EOL;
		$headers .= "Content-Type: text/html; charset=UTF-8".PHP_EOL;
		$headers .= 'From: تماس با ما <'.$sendemail.'>' . "\r\n";
		//subject

...
class ThemexUser {
	...
	public static function getAvatar($avatar, $user, $size, $default, $alt) {
		...
		// Check if wp_get_attachment_image_src returned false
		if ($default === false || !isset($default[0])) {
			$avatar_array = array('avatar.png', 'avatar1.png', 'avatar2.png', 'avatar3.png');
			$default = array();  // Initialize as array to avoid warnings
			$default[0] = THEME_URI . 'images/' . $avatar_array[array_rand($avatar_array)];
		}
		
		return '<img src="' . esc_url($default[0]) . '" class="avatar" width="' . intval($size) . '" alt="' . esc_attr($alt) . '" />';
	}
	...
	public static function renderBiography($user) {
		$out='<table class="form-table form-tabledes"><tbody><tr>';
	...
	public static function loginUser() {
		...
		if(is_wp_error($user) || empty($creds['user_login']) || empty($creds['user_password'])){
			...
			self::renderMessages('<a href="'.get_permalink().'" class="redirect"></a>');
		}
	...
	public static function registerUser() {
		...
		if(empty($userdata['user_email']) || empty($userdata['user_email']) || empty($userdata['user_password']) || empty($userdata['user_password_repeat']) || empty($userdata['user_gender']) ) {
			...
			if(!is_email($userdata['user_email'])) {
				...
				self::$data['messages'][]=__('This email is already in use.','academy').'در صورتی که این ایمیل متعلق به شما است و رمز عبور خود یا نام کاربری خود را فرآموش کرده اید، میتوانید از طریق'.'<a target="_blank" href="'.wp_lostpassword_url().'"> این صفحه </a>'.' رمز عبور خود را تغییر دهید ';
			}
		...
		if(!empty(self::$data['messages'])){
			...
			update_user_meta($user, 'user_gender', $userdata['user_gender'] );
			...
			if(empty($mail_message)) {
				$mail_message='با عرض سلام خدمت شما, %username%! به سایت '.get_bloginfo('name').' خوش آمدید .';
			}
			...
			if ( class_exists( 'WooCommerce' ) ) {
				...
				$user=new WP_User($user);
				$userid=$user->ID;
				$mailer->customer_new_account( $userid , ' ');
	...
	public static function recoverUser() {
		...
		if(email_exists(sanitize_email($userdata['user_email']))) {
			...
			if(!$allow || is_wp_error($allow)) {
				...
				if(empty($mail_message)) {
					$mail_message=' %username%! , سلام '."\r\n\r\n";
					$mail_message.=' برای تنظیم مجدد رمز عبور خود بر روی لینک زیر کلیک نمایید :'."\r\n\r\n".' %link%';
				}
				
				$mail_message=str_replace('%username%', $user->user_login, $mail_message);
				$mail_message=str_replace('%link%', $link, $mail_message);
				
				if(self::sendEmail($user_email, __('Password Recovery','academy'), $mail_message)) {
					self::renderMessages(__('Password reset link is sent','academy'));
				} else {
					self::$data['messages'][]=__('در ارسال ایمیل مشکلی پیش آمده','academy');
				}				
			}
	...
	if(!isset($_POST['viewProfile'])) {
			update_user_meta(self::$data['user']->ID, 'viewProfile', 'no' );
			} else  { update_user_meta(self::$data['user']->ID, 'viewProfile', 'yes' );}
	}
	
	// save new profile fields
	if(isset($_POST['user_uni'])) { update_user_meta(self::$data['user']->ID, 'user_uni', $_POST['user_uni'] );}
	if(isset($_POST['user_field'])) { update_user_meta(self::$data['user']->ID, 'user_field', $_POST['user_field'] );}
	if(isset($_POST['user_age'])) { update_user_meta(self::$data['user']->ID, 'user_age', $_POST['user_age'] );}
	if(isset($_POST['user_city'])) { update_user_meta(self::$data['user']->ID, 'user_city', $_POST['user_city'] );}	
	if(isset($_POST['user_job'])) { update_user_meta(self::$data['user']->ID, 'user_job', $_POST['user_job'] );}
	if(isset($_POST['billing_phone'])) { update_user_meta(self::$data['user']->ID, 'billing_phone', $_POST['billing_phone'] );}
	if(isset($_POST['user_gender'])) { update_user_meta(self::$data['user']->ID, 'user_gender', $_POST['user_gender'] );}
	
	if(isset($_POST['user_first_name'])) { update_user_meta(self::$data['user']->ID, 'billing_first_name', $_POST['user_first_name'] );}
	if(isset($_POST['user_last_name'])) { update_user_meta(self::$data['user']->ID, 'billing_last_name', $_POST['user_last_name'] );}
		...
		if (isset($_FILES['avatar']) && $_FILES['avatar']['name']!='') {
			...
			if ($filetype['ext']!='png' && $filetype['ext']!='jpg' && $filetype['ext']!='jpeg') {
				self::$data['messages'][]=__('فقط تصاویر با پسوند JPG و PNG مجاز به انتخاب میباشند.','academy');
		...
		if(isset($_POST['user_email']) ) { 
			if ( $_POST['user_email'] != self::$data['user']->user_email ) {
			...
			wp_update_user( array ( 'ID' => self::$data['user']->ID, 'user_email' => $_POST['user_email'] ) ) ;			
			update_user_meta(self::$data['user']->ID, 'billing_email', $_POST['user_email'] );
			
			self::$data['user']->user_email=$_POST['user_email'];
			}
	...
	public static function sendEmail($to, $subject, $message) {
		...
		$headers .= 'From: موسسه طلوع <info@toluehagh.ir>' . "\r\n";		
	...
	public static function getFulllastName($user) {
		$out='';
	...
	//Get description
	public static function getDescription($description='' ,$max_chard='500' , $more_linkd='') { 
	$contentd = str_replace( ']]>', ']]&gt;', $description );	
	$contentd = strip_tags( $contentd );
	
	if (strlen($contentd) == 0 ) { $contentd = 'اطلاعاتی موجود نیست' ; } else if (strlen($contentd) > $max_chard ) {  $contentd = mb_substr($contentd, 0, $max_chard , "utf-8" ).'...'; }

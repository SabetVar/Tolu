...
class ThemexInterface {
	...
	public static function renderOption($option) {
		...
		switch($option['type']) {
			...
			//Zip File for lessons uploader

			case 'allattachments':
				
				$out.='<input type="text" id="'.$option['id'].'" name="'.$option['id'].'" value="'.$value.'"></div>';
				
				if ($value) {
						
						preg_match_all('!\d+!', $value, $matches);
						$editpostid=$matches[0][0];
						
						if( is_string( get_post_status( $editpostid )) == true ){
							
							$editpostlink = get_edit_post_link($editpostid);
							$out.='<a style="float: left;" href="'.$editpostlink.'" target="_Blank" class="button">'.__('edit','academy').'</a>';
							
						}else{
							$postsl = get_posts(array("post_type"=>"wpdmpro","meta_key"=>"__wpdm_legacy_id","meta_value"=>$editpostid));
							$datap = (array)$postsl[0];
							if(isset($datap['ID'])){
								$editpostlink = get_edit_post_link($datap['ID']);
								$out.='<a style="float: left;" href="'.$editpostlink.'" target="_Blank" class="button">'.__('edit','academy').'</a>';
							}
						};
				}
			...
			case 'attachments':			
				...
				if ($value) {
					...
					foreach($value_arr as $row) {		
						...
						$editurl = $row['url'];
						preg_match_all('!\d+!', $editurl, $matches);
						$editpostid=$matches[0][0];
						...
						$out.='<a style="float: left;" href="'.$editpostlink.'" target="_Blank" class="button">'.__('edit','academy').'</a><a style="float: left;" href="#" class="button image-button repeatable-upload">'.__('Browse','academy').'</a><a style="float:left;" class="repeatable-remove button" href="#">'.__('Remove','academy').'</a></td></tr>';
						$i++;
			...
			case 'notif':			
				...
				$out.='<a class="repeatable-add button" style="float:right;" href="#">'.__('Add Field','academy').'</a><input type="hidden" id="'.$option['id'].'" name="'.$option['id'].'" value="" />';								
				...
				if ($value) {
					...
					foreach($value_arr as $row) {		
						...
						$out.='<td><input placeholder="'.__('Title','academy').'" type="text" name="'.$option['id'].'['.$i.'][title]" id="'.$option['id'].'['.$i.'][title]" class="'.$option['class'].'" value="'.stripslashes(htmlspecialchars($row['title'], ENT_QUOTES)).'" size="30" style="width:90%; margin-right: 2%; float:left;" /> <a style="float:left;margin-left:2%;" class="repeatable-remove button" href="#">'.__('Remove','academy').'</a> </td></tr>';
					...
					$out.='<td><input placeholder="'.__('Title','academy').'" type="text" name="'.$option['id'].'[0][title]" id="'.$option['id'].'[0][title]" class="'.$option['class'].'" value="" size="30" style="width:90%; margin-right: 2%; float:left;" /> <a style="float:left;margin-left:2%;" class="repeatable-remove button" href="#">'.__('Remove','academy').'</a> </td></tr>';}
			...
			case 'skytempc':
				...
				if ($valskys =='yes'){	
					...
					$contentshow = skyrcall('getRooms');
					$contentshow_res=$contentshow['result'];
					//$json_encodecon = json_encode($contentshow, JSON_PRETTY_PRINT | JSON_UNESCAPED_UNICODE | JSON_UNESCAPED_SLASHES );
					
					$out.='<select id="'.$option['id'].'" name="'.$option['id'].'" '.$attributes.'>';
					if(is_array($option['options'])) {
						foreach ($contentshow_res as $cresul){
							$selected='';
							if($cresul['id']==$value) {
								$selected='selected="selected"';
							}
							$out.='<option value="'.$cresul['id'].'" '.$selected.'>'.$cresul['title'].'</option>';
						}
			...
			case 'users':
				...
				$out.='<input type="number" name="add_user_id" id="add_user_id" class="">';
				...
				$out.='<div class="clear"></div><div class="hidepresent wp-core-ui button">عدم نمایش حضور و غیاب ها</div>';

				

				$out.='<a class="hidepresent wp-core-ui button" target="_blank" href="'.get_template_directory_uri().'/downloaduser.php?action='.$post->ID.'&source=local">دانلود اکسل دانشجویان حضوری کلاس</a><div class="clear"></div>';
	...
	$out.='<table id="table_results" class="data"> <thead><tr> <th >ردیف</th> <th>نام کاربری</th> <th>پست الکترونیکی</th>  <th>نام </th><th>نام خانوادگی </th><th>شماره تماس </th></tr></thead>' ;
	...
	$classnumber = ThemexCourse::getClassNumber($post->ID) ;
    ...
    foreach ($users as $user) {
		...
		$phoneu= get_phone_user_migrate($user);
		...
		// '<td>'. date("j/n/Y", strtotime($registered)). '</td>'.	 
		 ...
		 $out.= '<tr id="present_list" class="present_list"><td colspan="6">';		 

		 

		 $classzero = 1 ;

		 $userpresents = ThemexCourse::getUserPresentClass($user->ID,$post->ID);	 
		 ...
		 while($classzero <= $classnumber) {
			 ...
			 if (in_array( $classzero, $userpresents)) { $checked ='checked = "checked" ';}

			$out.= '<label><input '. $checked .' type="checkbox" id="cbox'.$classzero.'" name="present['.$post->ID.']['.$user->ID.']['.$classzero.']" value="checked">'.$classzero.'</label>';
	...
	$out.='</tbody></table>';
			...
			case 'usersb':
				...
				$users=ThemexCore::parseMeta($post->ID, $post->post_type, 'usersb');
				...
				$out.='<input type="number" name="add_userb_id" id="add_user_id" class="">';



				$out.='<input id="userb_totalpay" class="textfield" type="text" value="" placeholder="توضیحات" style="width: 200px;" name="user_pay"></input>';

				$out.='<input type="submit" name="add_userb" style="float:left;margin:0 0 10px 3px; width:auto;" class="button" value="'.__('Add','academy').'" /><div class="clear"></div>';	
				...
				if(!empty($users)) {
					...
					$out.='<input type="submit" name="remove_userb" style="float:left;margin-left:3px; width:auto;" class="button" value="'.__('Remove','academy').'" />';
	...
	$out.='<table id="table_results" class="data"> <thead><tr> <th>ردیف</th> <th>نام کاربری</th> <th>پست الکترونیکی</th>  <th>نام </th><th>نام خانوادگی </th><th>شماره تماس </th><th>مبلغ پرداختی</th></tr></thead>' ;
    ...
    foreach ($users as $user) {
		...
		$payuser = $user->_user_pay;

		if ($payuser!='') { 

		parse_str($payuser, $arruser);

			if (is_array($arruser)) { 

				foreach ($arruser as $courseuser => $paypice) {

				if ($courseuser== $post->ID ) {$payfees= $paypice;}
        ...
        $out.= '<tr style="background:#fffca5;">'.
		...
		// '<td>'. date("j/n/Y", strtotime($registered)). '</td>'.	 
		 ...
		 $out.= '<tr id="present_list" class="present_list" style="background:#fffca5;"><td colspan="7">';		 
	...
	$out.='</tbody></table>';
			...
			case 'usersv':
				...
				$users=ThemexCore::parseMeta($post->ID, $post->post_type, 'usersv');
				...
				$out.='<input type="number" name="add_userv_id" id="add_user_id" class="">';



				$out.='<input type="submit" name="add_userv" style="float:left;margin:0 0 10px 3px; width:auto;" class="button" value="'.__('Add','academy').'" /><div class="clear"></div>';	
				...
				if(!empty($users)) {
					...
					$out.='<input type="submit" name="remove_userv" style="float:left;margin-left:3px; width:auto;" class="button" value="'.__('Remove','academy').'" />';
	...
	$out.='<a class="hidepresent wp-core-ui button" target="_blank" href="'.get_template_directory_uri().'/downloaduser.php?action='.$post->ID.'&source=virt">دانلود اکسل دانشجویان مجازی کلاس</a><div class="clear"></div>';
	...
	$out.='</tbody></table>';
			...
			//Virtual users manager borse

			case 'usersvb':

				$users=ThemexCore::parseMeta($post->ID, $post->post_type, 'usersvb');
				...
				$out.='<input type="number" name="add_uservb_id" id="add_user_id" class="">';



				$out.='<input id="userb_totalpay" class="textfield" type="text" value="" placeholder="توضیحات" style="width: 200px;" name="uservb_pay"></input>';

				$out.='<input type="submit" name="add_uservb" style="float:left;margin:0 0 10px 3px; width:auto;" class="button" value="'.__('Add','academy').'" /><div class="clear"></div>';	
				...
				if(!empty($users)) {
					...
					$out.='<input type="submit" name="remove_uservb" style="float:left;margin-left:3px; width:auto;" class="button" value="'.__('Remove','academy').'" />';
...
//users manager
			...
			case 'senduser':
				...
				$out.='<textarea id="sendtext" name="sendtext"></textarea>';			

				$out.='<input name="add_numbers" placeholder="شماره های اضافه" id="add_numbers" class="" type="text">';

				$out.='<input name="add_emails" placeholder="ایمیل های اضافه" id="sendemail" class="" type="text" >';

				$out.='<div class="clear"></div>';

				$out.='<input name="sendsmsbut" style="float:left;margin:0 0 10px 3px; width:auto;" class="button" value="ارسال پیامک" type="submit">';

				$out.='<input name="sendemailbut" style="float:left;margin:0 0 10px 3px; width:auto;" class="button" value="ارسال ایمیل" type="submit">';							
...
$in['wpeditimage_disable_captions']=true;
 ...
 $in['toolbar1']='bold,italic,strikethrough,bullist,numlist,blockquote,hr,alignleft,aligncenter,alignright ';
...
add_filter('tiny_mce_before_init', 'myformatTinyMCE' );
	...
	public static function create_bootstrap_menu( $theme_location ) {
		...
		if ( ($theme_location) && ($locations = get_nav_menu_locations()) && isset($locations[$theme_location]) ) {
			...
			$menu_list .= '<div class="container-fluid mobile-menu">' ."\n";			   		 
			...
			foreach( $menu_items as $menu_item ) {
				...
				if( $menu_item->menu_item_parent == 0 ) {
					...
					if( $bool == true && count( $menu_array ) > 0 ) {
						...
						$menu_list .= '<a href="#" class="dropdown-toggle ripple" data-toggle="dropdown" role="button" aria-haspopup="true" aria-expanded="false">' . $menu_item->title . ' <span class="dropdownicon close-menu"></span></a>' ."\n";
						...
						$menu_list .= '<a class="ripple" href="' . $menu_item->url . '">' . $menu_item->title . '</a>' ."\n";

...
class ThemexCore {
	...
	public static function savePost($post_id) {
		...
		// Present Check For Students of Courses
		if(isset($_POST['present'])) {
			
			foreach ($_POST['present'] as $postid => $userArray) {
	
				$CourseID = $postid ;

				foreach( $userArray as $userID => $userClassArray) {
				$CourseUserID = $userID;				
				$class_list = implode(', ', array_keys($userClassArray));
						
				update_user_meta( $CourseUserID , 'PresentCourse_'.$CourseID, $class_list);
				...
				foreach ($meta_box['options'] as $option) {			
					...
					if(isset($_POST[$option['id']])) {
						...
						if(is_array($_POST[$option['id']])) {
							...
							$option['value']=urldecode(http_build_query($value));
						} else {

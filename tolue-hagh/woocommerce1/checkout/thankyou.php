...
if ($order) {
	...
	$itemv=ThemexWoo::getRelatedItemv($order->id);
...
<div class="column threecol">
	<div class="regbox">
		...
		if ($itemv->post_type=='course') {
				get_template_part('loopv', $itemv->post_type);
		} else if ($item->post_type=='course') {
...
?>
	...
	<div class="column fivecol">
		...
		<h3 class="reghead">سفارش شما ثبت شد.
         <?php if ($order->payment_method == 'bacs') { echo ' برای تکمیل روند ثبت نام از طریق مشخصات ارائه شده ذیل پرداخت خود را انجام دهید. ' ;} ?> 
         </h3>		
		...
		<li class="date">
            ...
            <?php if (function_exists(jdate)) {echo  jdate('Y-m-d', strtotime( $order->order_date ) ) ; } else {echo  date('Y-m-d', strtotime( $order->order_date ) ) ; } ?>
		</li>
		<li class="total">

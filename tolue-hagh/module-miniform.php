<form action="<?php echo ThemexCourse::getAction(get_permalink()); ?>" method="POST">
    ...
    <?php  $butttextcap = 'ظرفیت حضوری تکمیل';
		$butttextcapr = ThemexCourse::$data['course']['butttextcap'] ;
		if ($butttextcapr !=''){$butttextcap = $butttextcapr; } 
	 ?>        
    <span class="caption"> <?php echo $butttextcap ; ?></span>  </div>
		<?php }
	 else if(!ThemexCourse::isMember() && !ThemexCourse::isMemberb() ) { ?>
		<?php if(ThemexCourse::$data['course']['status']!='private') { ?>
        ...
        <?php $butttext = ThemexCourse::$data['course']['butttext']; ?>
			<span class="caption"><?php if ( $butttext !=''){ echo $butttext; } else { echo 'حضوری';} ?></span>
    ...
    <?php  $butttextaf = 'دانشجوی حضوری';
		$butttextafr = ThemexCourse::$data['course']['butttextaf'] ;
		if ($butttextafr !=''){$butttextaf = $butttextafr; } 
	 ?>
    <span class="caption"> <?php echo $butttextaf;  ?> </span>  </div>
     <?php /*
...
<form action="<?php echo ThemexCourse::getAction(get_permalink()); ?>" method="POST">
    ...
    <?php  $butttextcapv = 'ظرفیت ثبت نام تکمیل شده';
		...
		if ($butttextcaprv !=''){$butttextcapv = $butttextcaprv; } 
	 ?>        
    <span class="caption"> <?php echo $butttextcapv ; ?></span>  </div>
		<?php }
	else if(!ThemexCourse::isMemberv() && !ThemexCourse::isMembervb() ) { ?>
		<?php if(ThemexCourse::$data['course']['statusv']!='private') { ?>
		<input type="hidden" name="course_action" value="addv" />
		<a href="#" class="button medium price-button submit-button left">
			<span class="caption"><?php _e('مجازی', 'academy'); ?></span>
    ...
    <div class="button medium secondary left">
    <span class="caption"> دانشجوی مجازی</span>
    </div>
         <?php /*
        <span class="note-s" > در صورتی که هنوز مبلغ دوره را پرداخت نکرده اید، از قسمت زیر استفاده کنید. </span>
    		<input type="hidden" name="course_action" value="addv" />
